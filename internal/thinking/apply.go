...
func applyUserDefinedModel(body []byte, modelInfo *registry.ModelInfo, fromFormat, toFormat, providerKey string, suffixResult SuffixResult, summaryConfig SummaryConfig) ([]byte, error) {
	...
	// in standardLevelOrder are handled automatically. Runs before
	// normalizeUserDefinedConfig so the level is corrected while still in
	// ModeLevel (normalize may convert level to budget for budget-capable providers).
